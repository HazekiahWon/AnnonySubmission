...
def get_data(data_arg, tokenizer: transformers.PreTrainedTokenizer, template_variation: bool, ratio=0):
    ...
    for k, data_path in data_config.items():
        ...
        if os.path.exists(data_path):
            ...
            if data_path.endswith(".json"):
                ...
                if len(logp_dict)>0:
                    for item in list_data_dict:
                        ...
                        if tmp is not None: 
                            ...
                            rlist.extend(tmp[1].tolist())
                else:
                    for item in list_data_dict:
                        is_syn = not item['source'].startswith('self')
                        item['reward_type'] = 'self' 
                        item['raw_reward'] = float(not item.get('neg', False))
                        item['logp'] = None # np.array([0.0])
                print(item)
                print('------------')
                print(f"{haslogp} logp in {len(list_data_dict)} data; neg count {syn_neg} in {nologp} syn data")
                print('------------')
                ...
                for iidx,(q,idxlist) in enumerate(qset.items()):
                    allneg = sum([list_data_dict[idx].get('neg', False) for idx in idxlist])==len(idxlist)
                    allcor = sum([list_data_dict[idx].get('neg', False) for idx in idxlist])==0
                    ...
                    notsyn_rewards = np.asarray([list_data_dict[idx]['raw_reward'] for idx in idxlist], np.float32)
                    ...
                    std = 1.0 if allcor else np.std(notsyn_rewards)
                    mean = 0.0 if allcor else np.mean(notsyn_rewards)
                    
                    erewards = []
                    for idx in idxlist:
                        erew = list_data_dict[idx].get('estimated_reward')
                        if erew is not None: 
                            erewards.extend(erew.tolist())
                    emean = np.mean(erewards) if len(erewards)>0 else 0.0
                    estd = np.std(erewards) if len(erewards)>0 else 1.0
                    
                    # rewards = []
                    for idx in idxlist: 
                        list_data_dict[idx]['group_idx'] = iidx 
                        r = list_data_dict[idx]['raw_reward']
                        
                        if list_data_dict[idx].get('reward_type', None)=='self':
                            r = (r-mean)/std
                            list_data_dict[idx]['reward'] = np.clip(r, -2.0, 2.0) if data_args.is_weighted_loss==3 else r
                        else: 
                            list_data_dict[idx]['reward'] = r
                         
                        final_data.append(list_data_dict[idx])
                            
                print(f"drop {num_an} all-negatives")
                list_data_dict = final_data
        ...
        #     list_data_dict = [x for x in list_data_dict if (not x.get('neg', False) and not use_neg)]
        print(f"invalid samples removed, now data num {len(list_data_dict)}")
    ...
    for eid, example in enumerate(all_dat):
        ...
        if model_args.round_id>0: # in round>0, we use logp, so we must make sure the prompt context aligns
            ...
            if model_args.deepseek_templ==1 and 'req' in example:
                sources.append(f"{example['req']}")
                targets.append(f"{example['output']}{tokenizer.eos_token}")
            else:
        ...
        tmp = example.get('estimated_reward', None)
        if data_args.is_weighted_loss==5: 
            estimated.append(tmp if tmp is None else (tmp-rmean)/rstd)
        else:
            estimated.append(tmp if tmp is None else (tmp-0.75)*2) # [-1,1]
    ...
    print_on_rank_0("sanity check on rewards: among first 500 samples")
    pos_rewards = [p for o,p in zip(outcomes[:500],weights[:500]) if o>0.5]
    np.mean([p for o,p in zip(outcomes,weights) if o<0.5])
    neg_rewards = [p for o,p in zip(outcomes[:500],weights[:500]) if o<0.5]
    syn_neg = np.sum([p is None for o,p in zip(outcomes[:500],logps[:500]) if o<0.5])
    print_on_rank_0(f"pos mean {np.mean(pos_rewards)}, neg mean {np.mean(neg_rewards)}, syn neg {syn_neg}")
...
class WeightedLossTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if use_estimated_reward:
            rewards_ = [weigh if weigh is None else torch.tensor(weigh, dtype=torch.float32).to(logits.device) for weigh in weights]
        else: 
        ...
        token_logp = -tmp 
        if not use_estimated_reward: sample_rewards = rewards_.view(num_sample, 1)
        if has_reflogp:
            ...
            if use_estimated_reward:
                sample_rewards = torch.ones_like(token_logp) 
                ...
                if isself:
                    ...
                    if use_estimated_reward:
                        ...
                        if r is not None: 
                            ...
                            leng = min(len(a),len(r))
                            sample_rewards[i][m==1][:leng] = r[:len(a)]
                        else: 
                            sample_rewards[i][m==1] = -1.0 if outcomes[i]<0.5 else 1.0
        ...
        for ii, (r, mm, lp, ls) in enumerate(zip(outcomes, mask, sample_logp, sample_rho_loss)):
            ver = 'neg' if r<0.5 else 'pos'
            if token_reflogps[ii] is None: 
                prefix = 'syndata_'
                ver = prefix + ver
...
def train():
    ...
    elif  data_args.is_weighted_loss>0 and model_args.show_kl==1:
        TrainerClass = WeightedLossTrainer
